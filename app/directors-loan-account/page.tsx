...
export default async function DirectorsLoanAccountPage({
  ...
  searchParams: Promise<{ trial_balance_id?: string }>;
}) {
  const { trial_balance_id } = await searchParams;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="p-8">
        ...
        <div className="mt-8 rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
          <h2 className="text-lg font-bold text-slate-900">All Records ({rows.length})</h2>
          <div className="mt-4 space-y-2">
            {rows.map(({ dla, result }) => (
            ...
            {rows.length === 0 && (
              <p className="text-sm text-slate-500 text-center py-8">No DLA records yet.</p>
            )}
          </div>
        </div>
