...
async function undoDisposal(id: string) {
  "use server";
  await supabase.from("fixed_assets").update({
    disposal_date: null,
...
export default async function FixedAssetsRegisterPage({
  ...
  const EditForm = ({ asset }: { asset: any }) => (
    <div className="border-t border-slate-100 p-4 bg-slate-50">
      <form action={updateAsset.bind(null, asset.id)} className="grid gap-4 md:grid-cols-3">
        ...
        <div className="md:col-span-3 flex items-center gap-3">
          ...
          {asset.disposal_date && (
            <span className="text-xs text-slate-400">Clear the Disposal Date above and save to undo the disposal, or use "Undo Disposal" for a one-click reset.</span>
          )}
        </div>
      </form>
    </div>
            ...
            {activeAssets.map((asset) => {
              ...
              return (
                <div key={asset.id} className="rounded-xl border border-slate-100">
                  ...
                  {isEditing && <EditForm asset={asset} />}
                </div>
              );
            })}
          ...
          <div className="mt-6 rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
            ...
            <div className="mt-4 space-y-2">
              {disposedAssets.map((asset) => {
                ...
                return (
                  <div key={asset.id} className="rounded-xl border border-slate-100 opacity-90">
                    <div className="flex items-center justify-between p-4">
                      <div className="flex-1">
                        <p className="font-semibold text-slate-900">{asset.description}</p>
                        <p className="text-xs text-slate-500 mt-0.5">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <form action={undoDisposal.bind(null, asset.id)}>
                          <button className="rounded-lg bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700 hover:bg-amber-100 transition-colors">
                            Undo Disposal
                          </button>
                        </form>
                        <form action={deleteAsset.bind(null, asset.id)}>
