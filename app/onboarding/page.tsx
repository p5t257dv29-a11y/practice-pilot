...
// Helper function to format currency (e.g. £1,000.00)
...
// Helper function to format counts with commas (e.g. 1,250)
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <a
            href={statusFilter === "Pending" ? "/onboarding" : "/onboarding?status=Pending"}
            className={statCardClass(statusFilter === "Pending")}
          >
            <p
              className={`text-2xl font-bold ${
                statusFilter === "Pending" ? "text-white" : "text-slate-900"
              }`}
            >
            ...
            <p
              className={`text-xs mt-1 ${
                statusFilter === "Pending" ? "text-slate-300" : "text-slate-500"
              }`}
            >
              Pending
            </p>
          ...
          <a
            ...
            className={statCardClass(statusFilter === "In Progress")}
          >
            <p
              className={`text-2xl font-bold ${
                statusFilter === "In Progress" ? "text-white" : "text-blue-600"
              }`}
            >
            ...
            <p
              className={`text-xs mt-1 ${
                statusFilter === "In Progress" ? "text-slate-300" : "text-slate-500"
              }`}
            >
              In Progress
            </p>
          ...
          <a
            href={statusFilter === "Complete" ? "/onboarding" : "/onboarding?status=Complete"}
            className={statCardClass(statusFilter === "Complete")}
          >
            <p
              className={`text-2xl font-bold ${
                statusFilter === "Complete" ? "text-white" : "text-green-600"
              }`}
            >
            ...
            <p
              className={`text-xs mt-1 ${
                statusFilter === "Complete" ? "text-slate-300" : "text-slate-500"
              }`}
            >
              Complete
            </p>
        ...
        {isFiltered ? (
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
            <h2 className="text-lg font-bold text-slate-900">
              Results ({formatNumber(filteredRequests.length)})
            </h2>
                ...
                <div key={request.id} className="rounded-xl border border-slate-100 p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {/* Display estimated fee if field exists on request */}
                      {request.estimated_fee && (
                        <p className="text-xs font-medium text-slate-700 mt-1">
                          Est. Fee: {formatCurrency(request.estimated_fee)}
                        </p>
                      )}
