...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error || !comp) {
      console.error("Computation lookup failed:", error?.message);
    ...
    if (updateError) {
      console.error("Computation update failed:", updateError.message);
      return NextResponse.json({ error: "Failed to update computation" }, { status: 500 });
    }
    ...
    console.error("send-tax-computation route crashed:", err);
    return NextResponse.json(
